/*
 * @author:  Allan Jair Escamilla Hernández, Raúl González Portillo y César Mauricio Arellano Velásquez
 ...
 */
#include"dijkstra.h" // Incluimos el archivo que contiene las estructuras y la implementación del algoritmo de Dijkstra
    ...
    if(*Raiz == NULL)
        *Raiz = temp;
        ...
        do{
            ...
            if (aux == NULL)
                printf("El nodo ingresado no existe en el grafo, asegúrese de ingresar uno que realmente exista!\n\n");
        } while (aux == NULL);
...
/* * Función que busca un nodo dado por el usuario
   ...
   * @param int cant recibe una variable que contara las veces que se acceda a cada conexión de un nodo.
   * @param int total recibe el total de conexiones por nodo.
   * @param char buscado recibe la etiqueta del nodo a buscar.
   * @param Nodo** aux recibe la dirección de memoria de un apuntador que almacenará el nodo buscado.
*/
void buscarNodo(Nodo* Raiz, char buscado, Nodo** aux){
    Nodo* temp = Raiz;
    while (temp != NULL){
...
/* * Función que despliega la tabla de calculada por el algoritmo de Dijkstra.
   * @param Tabla tabla recibe la tabla donde se calcularon los caminos más cortos.
